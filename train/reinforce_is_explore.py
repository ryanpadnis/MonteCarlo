...
def resolve_env_name(env_name: str) -> str:
    env_aliases = {
        "FetchPush-v3": "FetchPush-v4",
    }
    alias = env_aliases.get(env_name)
    if alias in gym.envs.registry:
        return alias
    if env_name in gym.envs.registry:
        return env_name
    ale_name = f"ALE/{env_name}"
    if ale_name in gym.envs.registry:
        return ale_name
    return env_name
...
# Space helpers
# ---------------------------------------------------------------------------
def obs_dim_from_space(space: spaces.Space) -> int:
    if isinstance(space, spaces.Discrete):
        return int(space.n)
    if isinstance(space, spaces.Box):
        return int(np.prod(space.shape))
    if isinstance(space, spaces.Dict):
        return sum(obs_dim_from_space(s) for _, s in sorted(space.spaces.items()))
    raise TypeError(f"Unsupported observation space: {space}")


def preprocess_obs(obs, space: spaces.Space) -> np.ndarray:
    if isinstance(space, spaces.Discrete):
        x = np.zeros(space.n, dtype=np.float32)
        x[int(obs)] = 1.0
        return x
    if isinstance(space, spaces.Box):
    ...
    if isinstance(space, spaces.Dict):
        return np.concatenate([
            preprocess_obs(obs[k], s)
            for k, s in sorted(space.spaces.items())
        ]).astype(np.float32)
    raise TypeError(f"Unsupported observation space: {space}")


def action_dim_from_space(space: spaces.Space) -> int:
    if isinstance(space, spaces.Discrete):
        return int(space.n)
    if isinstance(space, spaces.Box):
        return int(np.prod(space.shape))
...
class PolicyNet(nn.Module):
    """Shared-trunk actor-critic for discrete and Box action spaces."""
    ...
    def __init__(
        ...
        self.act_dim = action_dim_from_space(action_space)
        self.trunk = nn.Sequential(
            nn.Linear(obs_dim, hidden), nn.Tanh(),
            nn.Linear(hidden, hidden),  nn.Tanh(),
        )
        self.actor_head  = nn.Linear(hidden, self.act_dim)
        self.log_std = (
            None if self.is_discrete
            else nn.Parameter(torch.zeros(self.act_dim))
        )
        ...
        if not self.is_discrete:
            ...
            self.register_buffer("action_low", torch.as_tensor(low))
            self.register_buffer("action_high", torch.as_tensor(high))

    def forward(self, x: torch.Tensor):
        h = self.trunk(x)
        return self.actor_head(h), self.critic_head(h).squeeze(-1)

    def _continuous_params(self, x: torch.Tensor):
        mean, _ = self(x)
        mean = torch.nan_to_num(mean).clamp(-MEAN_CLAMP, MEAN_CLAMP)
        log_std = torch.nan_to_num(
            self.log_std, nan=0.0,
            posinf=LOG_STD_MAX, neginf=LOG_STD_MIN,
    ...
    def get_dist(self, x: torch.Tensor):
        ...
        mean, std = self._continuous_params(x)
        return Normal(mean, std)

    def _raw_to_env_action(self, raw_action: torch.Tensor) -> np.ndarray:
        unit_action = torch.tanh(raw_action)
        scaled = self.action_low + 0.5 * (unit_action + 1.0) * (
    ...
    def sample_action(self, x: torch.Tensor):
        ...
        if self.is_discrete:
            action = dist.sample()
            return action, int(action.item()), dist.log_prob(action), value
    ...
    def deterministic_action(self, x: torch.Tensor):
        if self.is_discrete:
            logits_or_mean, _ = self(x)
            return int(torch.argmax(logits_or_mean, dim=-1).item())
        mean, _ = self._continuous_params(x)
        return self._raw_to_env_action(mean.squeeze(0))
    ...
    def evaluate_actions(self, x: torch.Tensor, actions: torch.Tensor):
        ...
        if self.is_discrete:
            ...
            log_prob = self._squashed_log_prob(dist, actions)
...
def collect_rollout(
    ...
    for _ in range(n_steps):
        obs_vec = preprocess_obs(obs, obs_space)
    ...
    if policy.is_discrete:
        ...
        act_arr = np.array(act_buf, dtype=np.float32).reshape(len(act_buf), -1)
    ...
    return {
        "obs":       t(obs_arr),
        "actions":   t(act_arr, dtype=torch.long if policy.is_discrete
                       else torch.float32),
...
def train_on_batch(
    ...
    stats = {"pg_loss": [], "vf_loss": [], "ent": [],
             ...
             "kl_approx": [], "clip_frac": [], "skipped_updates": []}
    ...
    for _ in range(epochs):
        ...
        for start in range(0, n, batch_size):
            ...
            if log_prob_new.shape != lp_old.shape or log_prob_new.shape != adv.shape:
                raise RuntimeError(
                    "Policy log-prob shape mismatch: "
                    f"new={tuple(log_prob_new.shape)}, "
                    f"old={tuple(lp_old.shape)}, adv={tuple(adv.shape)}")
            if entropy.shape != adv.shape:
                raise RuntimeError(
                    "Policy entropy shape mismatch: "
                    f"entropy={tuple(entropy.shape)}, adv={tuple(adv.shape)}")
            if values.shape != ret.shape:
                raise RuntimeError(
                    "Value shape mismatch: "
                    f"values={tuple(values.shape)}, returns={tuple(ret.shape)}")
            ...
            if not torch.isfinite(loss):
                ...
                stats["skipped_updates"].append(1.0)
                continue
            loss.backward()
            ...
            optimizer.step()
            policy.sanitize_parameters()
            stats["skipped_updates"].append(0.0)
...
def evaluate(policy: PolicyNet, env_name: str, n_eps: int,
    ...
    env   = gym.make(resolve_env_name(env_name))
...
class ReinforceISTrainer:
    ...
    def train(self) -> None:
        ...
        env = RecordEpisodeStatistics(gym.make(resolve_env_name(self.env_name)))
        obs_dim = obs_dim_from_space(env.observation_space)

        policy    = PolicyNet(obs_dim, env.action_space).to(self.device)
    ...
    def save_renders(self, n_episodes: int = N_GIF_EPS) -> None:
        if self._policy is None:
            probe_env = gym.make(resolve_env_name(self.env_name))
            obs_dim = obs_dim_from_space(probe_env.observation_space)
            p = PolicyNet(obs_dim, probe_env.action_space).to(self.device)
            probe_env.close()
        ...
        env = gym.make(resolve_env_name(self.env_name), render_mode="rgb_array")
        obs_space = env.observation_space
        ...
        for ep in range(n_episodes):
            ...
            while not done:
                ...
                obs_vec = preprocess_obs(obs, obs_space)
                obs_t  = torch.as_tensor(
                    obs_vec, dtype=torch.float32,
                    device=self.device).unsqueeze(0)
                with torch.no_grad():
                    action = self._policy.deterministic_action(obs_t)
